...
import { MessageSquare, ListTodo, Search, Activity, Cpu, Wifi, Shield } from "lucide-react";
import ChatPanel from "./ChatPanel";
import TaskPanel from "./TaskPanel";
import SearchPanel from "./SearchPanel";
import { HUDCornerRings, HUDGridBackground, HUDScanLine } from "./HUDElements";

type Tab = "chat" | "tasks" | "search";
...
function SystemStatus() {
  ...
  return (
    <div className="flex items-center gap-3 text-[10px] font-mono text-hud-muted">
      ...
      <div className="flex items-center gap-1.5">
        <Cpu size={10} className="text-hud-cyan" />
        <span className="text-hud-cyan">AI READY</span>
      </div>
      <div className="flex items-center gap-1.5">
      ...
      <span className="text-hud-muted/50 hidden md:inline">
...
function SidebarTab({
  ...
  return (
    <button
      onClick={onClick}
      className={`
        w-full flex flex-col items-center gap-1.5 py-4 px-2 transition-all duration-200 relative
        font-mono text-[10px] tracking-wider
        ${active
          ? "text-hud-cyan bg-cyan-900/20 border-r-2 border-hud-cyan"
          : "text-hud-muted hover:text-hud-cyan/70 hover:bg-cyan-900/10"
        }
      `}
    >
      {active && (
        <span
          className="absolute left-0 top-0 bottom-0 w-0.5 bg-hud-cyan"
      ...
      <header className="relative z-10 border-b border-hud-cyan/30 bg-black/40 backdrop-blur-sm">
        <div className="flex items-center justify-between px-4 py-3">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1
                className="text-xl font-bold tracking-[0.3em] text-hud-cyan font-mono"
                style={{
                  textShadow:
              ...
              <p className="text-[10px] text-hud-muted tracking-widest font-mono">
          ...
          <div className="text-right font-mono flex-shrink-0">
            <div
              className="text-xl font-bold text-hud-cyan tracking-widest"
              style={{ textShadow: "0 0 8px rgba(0,212,255,0.6)" }}
            >
              {time}
            </div>
            <div className="text-[10px] text-hud-muted tracking-wider">
              {date}
            </div>
          </div>
        </div>

        {/* Sub-header: system metrics */}
        <div className="px-4 py-1.5 flex items-center gap-6 text-[10px] font-mono text-hud-muted/60 border-t border-hud-cyan/10 bg-black/20">
          ...
          <span>
            CORE TEMP:{" "}
            <span className="text-hud-cyan">98.6°F</span>
          </span>
          <span>
            SHIELD:{" "}
            <span className="text-hud-orange">ACTIVE</span>
          </span>
          <span>
            AI MODEL:{" "}
            <span className="text-hud-cyan">CLAUDE SONNET 4-6</span>
          </span>
          <span className="ml-auto text-hud-muted/40">
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="px-4 py-2 border-b border-hud-cyan/20 bg-black/20 backdrop-blur-sm flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div
                className="w-2 h-2 rounded-full bg-hud-cyan"
                style={{
